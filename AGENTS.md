...
- Removed pages `/pk.html`, `/pk` and `/pk/` must return exactly 404, as explicitly agreed after the deployed 410 rules still returned 404. Keep that strict expectation in the live check. Azure rejects `rewrite` combined with `statusCode` in `routes` (distinct from `responseOverrides`). `410.html` is a standalone informational page, not an HTTP-410 route. `/remotion/*` is blocked; missing pages use the 404 response override. Error-page assets use root-relative paths, and language detection must not redirect nested missing URLs.
- Azure treats `/pk` and `/pk/` as duplicate route definitions: configure only `/pk`, but keep both URL variants in the live deployment check. Trailing-slash variants must not be added as separate rules.
