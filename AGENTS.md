...
- Routing intends status-only 410 responses for `/pk.html`, `/pk` and `/pk/`; live effectiveness must pass the deployment check. Azure rejects `rewrite` combined with `statusCode` in `routes` (distinct from `responseOverrides`). `410.html` is a standalone page, not the route response body. `/remotion/*` is blocked; missing pages use the 404 response override. Error-page assets use root-relative paths, and language detection must not redirect nested missing URLs.
