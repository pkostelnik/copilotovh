...
test('live deployment check rejects an ineffective 410 rule', async t => {
  const config = require('../staticwebapp.config.json');
  t.mock.method(global, 'fetch', async url => {
    const status = new URL(url).pathname === '/pk.html' ? 404 : 200;
    return new Response('', { status, headers: config.globalHeaders });
  });
  await assert.rejects(verifyDeployment('https://example.invalid'), /\/pk.html: HTTP status/);
});
...
test('live deployment check rejects published documentation even with correct security headers', async t => {
  ...
  t.mock.method(global, 'fetch', async url => {
    ...
    const status = route.startsWith('/pk') ? 410 :
